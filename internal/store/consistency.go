...
func (s *Store) Consistency(ctx context.Context) (ConsistencyReport, error) {
	...
	if err := s.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM clicks c LEFT JOIN links l ON l.code=c.code WHERE l.code IS NULL AND c.code <> ""`).Scan(&out.OrphanClicks); err != nil {
