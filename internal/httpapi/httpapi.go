...
func (h *Handler) expire(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.link.Update(r.Context(), code, l.TargetURL, l.Description, time.Now().UnixMilli(), l.MaxClicks); err != nil {
		h.writeError(w, err)
		return
	}
...
func (h *Handler) writeError(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, link.ErrNotFound), errors.Is(err, click.ErrNotFound):
		h.writeJSON(w, http.StatusNotFound, errResp("link not found"))
	case errors.Is(err, link.ErrExpired), errors.Is(err, click.ErrExpired),
		errors.Is(err, link.ErrLimitReached), errors.Is(err, click.ErrLimitReached):
